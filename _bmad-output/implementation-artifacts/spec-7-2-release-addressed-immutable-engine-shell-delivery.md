...
**Amendment 2026-09-22 (Epic 7 retrospective): the publish now publishes the document last.** The retrospective found that the single unordered `aws s3 sync` in `.github/workflows/sync-game-release.yml` violated this story's own frozen *Always* clause "the pipeline publishes tree-first": the sync uploads objects individually and unordered, so `/engine/index.html` could land before the subresources it references, and because the route grants an addressed shell file `private, immutable, max-age=31536000` from the URL shape alone — while R2 selects the object from the path and ignores `?v=` — a client booting inside that window would keep the previous release's bytes under the new identifier for a year and never revalidate. The window is therefore **not** self-healing under immutable caching, contrary to the deferred-work entry's original description. The workflow now runs the shell publish as two ordered syncs — every shell object except `index.html` (and `build-metadata.json`) first, then `index.html` alone last — with `tests/workflow-manifest.test.ts` pinning both the order and the flags; the sync stays additive, and the corrected residual is recorded in `deferred-work.md`.
...
**Correction (2026-09-22, Story 7.5).** The statement above that "Playwright-driven Chromium cannot run this measurement: it does not serve these responses from the HTTP cache" does not reproduce. With a real server and no `page.route` interception, Playwright's Chromium **and** Firefox both serve an addressed shell subresource from the cache on the next navigation: `timing().responseStart` is `-1`, `transferSize` is `0`, and the preview's request log records no request for that URL. Story 7.5 asserts the reload metric from `transferSize` in both browsers and cross-checks it against the server log; the earlier conclusion was an instrumentation artefact (page-traffic interception disables the cache, and the `request` event list still names cache-served requests).
