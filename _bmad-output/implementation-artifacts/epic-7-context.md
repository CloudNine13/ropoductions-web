...
- **Owner gates:** OD-1 blocks 7.3, OD-2 blocks 7.2 — amortised asset-path validation and versioned immutable shell caching change documented contracts. Implementation cannot start before the decision; approval forces amending `AGENTS.md` and `save-and-runtime-contract.md` in the same cutover.
...
- **Shell delivery (7.2):** the shell document and every subresource URL carry `?v=<commitSha>` from `build-metadata.json`; `/play` requests the same addressed URL. Shell responses are `Cache-Control: private, immutable, max-age=31536000`; an unaddressed shell request is refused, never served from cached fill. Media (`img/ audio/ effects/ movies/ data/`) keeps `private, max-age=86400` — the additive sync reuses file names. A release changes every shell URL; rollback cannot mix versions.
