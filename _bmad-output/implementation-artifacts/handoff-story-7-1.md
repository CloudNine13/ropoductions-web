...
- E2E: Playwright, **Chromium only** (`playwright.config.ts:18-21`), `webServer` builds and serves the OpenNext worker on `127.0.0.1:3100`; session fixtures in `e2e/helpers/session.ts`; `/play` harness `e2e/save-hud.spec.ts`. Firefox project is Story 7.5's job.
