...
- **Age Gate Persistence:** Confirmed age status must be stored in a client-side cookie (`ropoductions_age_verified=true`) with a 14-hour lifetime (50,400 seconds, locked per project owner decision) so that returning visitors are not repeatedly prompted. Clearing browser storage resets the gate.
...
- **Multilanguage Localization:** Provide an accessible language switcher in the header and footer supporting initial locales (`EN` default, `JA`, `ES`, `RU`, `ZH`, `PL`). Switching languages must update all portal microcopy and notices instantly without requiring a full page refresh, persist the selection in a 365-day cookie (`ropoductions_lang`), and gracefully fall back to English for missing keys without rendering raw translation keys.
...
- **State & Cookie Conventions:**
  - Age Verification: `ropoductions_age_verified` (valid 14 hours / 50400s, `SameSite=Lax`, `Secure`, locked per owner decision).
...
- **First-Time Arrival Flow (21+ Gate):**
  ...
  - Confirming triggers a smooth 200ms ease-out dissolution of the modal, sets the 14-hour cookie, and restores user interaction with the page.
