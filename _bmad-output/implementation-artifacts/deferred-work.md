...
- source_spec: `_bmad-output/implementation-artifacts/spec-6-1-admin-panel-entry-point-role-safe-resolution.md`
  summary: The age gate skips its server re-render while the URL carries server-derived modal state (`paywall`, `auth_required`, `auth_error`), so an admin who lands on such a URL — reachable when `/play` bounces on a transient infrastructure failure while the session cookie is still valid — does not see the admin entry until a manual reload. Covering it needs the dismissed paywall modal to re-trigger the refresh after closing (cross-component plumbing), not a wider refresh, which resurrects dismissed dialogs. Reopen trigger: an admin reports a missing entry after a bounce, or the paywall modal gains a shared dismissal signal.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-6-1-admin-panel-entry-point-role-safe-resolution.md`
  summary: `/play` validates the caller's session twice per render — `play/page.tsx` calls `validateSessionAccess` for the paywall/tier decision and `resolveAdminAccess` calls it again for the chrome verdict (plus up to two `getPatronOverride` reads on the insufficient-pledge path). The resolver's frozen signature takes only a cookie reader, so sharing one validation result would mean changing a locked contract; the party accepted the duplicate co-located D1 read instead. Reopen trigger: `/play` TTFB regresses, or an in-isolate session/override cache lands for the asset route (spec-3-2 precedent).
