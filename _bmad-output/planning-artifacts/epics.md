...
### A-2026-09-23-01 — Story 5.5 audit-trail clauses operationally defined (story interpretation, pending owner ratification)

Story 5.5's acceptance criteria used two terms without defining them operationally — "within the same D1 interaction" and "exactly one trail row" — and the story was implemented without renegotiating the approved text. Dated story interpretation 2026-09-23, evidence `_bmad-output/implementation-artifacts/spec-5-5-grant-revoke-audit-trail.md` ("Operational definitions" and "Design Notes (measured)", with the corresponding review-triage rows): (a) **"within the same D1 interaction" means one atomic `db.batch([auditStatement, mutationStatement])`** — the audit statement is `batch[0]` and D1 runs a batch's statements sequentially inside one implicit transaction, measured on the real engine by forcing a CHECK violation and observing that neither table changed, so an unaudited mutation and an orphan trail row are both unreachable; (b) **"exactly one trail row" means one row per successful mutation call** — one per `upsertPatronOverride` (`grant` when the row did not exist, `update` otherwise, derived in SQL from the pre-write state by correlated subquery rather than passed by a caller whose earlier read is a separate await point and can go stale) and one per `deletePatronOverrideGuarded` that actually removed a row, so a revoke that removes nothing appends nothing while a creator-admin re-materialization (OAuth login / session validation) does append a row, because it genuinely rewrites `updated_at_sec`; denied or rejected attempts never reach the DAL and are deliberately unlogged; (c) the **4-eyes clause is satisfied by the pre-existing entry in `deferred-work.md`** (epic-5 retro, reopen trigger "more than a handful of operators hold admin keys"), which this story verifies and refreshes instead of duplicating; (d) the **trail is display-only** — nothing may read `override_audit` for an authorization decision, admin authority stays `patron_overrides` plus `CREATOR_ADMIN_PATREON_IDS` env bootstrap, and sealing stays keyed on `patron_overrides.granted_by`; (e) **attribution is a caller fact passed as a required DAL argument** (`actorPatronId`, with `SYSTEM_BOOTSTRAP_ACTOR` for non-human writers — bootstrap, sync, the dev seed CLI, the e2e harness), because a SQL trigger cannot know the acting admin: `granted_by` is INSERT-only by design, so an UPDATE trigger would re-create the F3 grantor-rewrite defect inside the trail. The story text above stands as originally approved; this note records the story's own reading of it — derived from the measured probes and the six-lens adversarial review recorded in the story artifact — and the owner ratifies or corrects these five definitions at review of Story 5.5 (PR #68). Until that ratification they are the implementer's interpretation, not an owner decision.
