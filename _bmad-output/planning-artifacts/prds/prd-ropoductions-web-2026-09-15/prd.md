...
* **Capability:** The system stores the confirmed age status in a secure client cookie (`ropoductions_age_verified=true`, max-age 14 hours / 50,400 seconds, locked per project owner decision) so that returning visitors are not repeatedly prompted within a 14-hour window.
* **Consequences (testable):**
  - Refreshing or reopening the browser within 14 hours bypasses the age modal.
...
### 6.1 In Scope for v1
* Mandatory 21+ age verification modal on entry with 14-hour cookie persistence (50,400s, locked per owner decision).
...
* **[ASSUMPTION §4.1 / FR-2]:** A 14-hour client-side age confirmation cookie (`max-age=50400`) is established and locked per project owner decision for legal compliance before re-prompting.
