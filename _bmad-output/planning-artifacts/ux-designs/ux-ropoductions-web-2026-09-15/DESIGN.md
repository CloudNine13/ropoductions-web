...
* **Animated Studio Emblem (`studio-logo-animated.gif`):** 128x128 transparent 27-frame pixel-art loop of the crystal leaf, hero slot only, rendered at 56px with `.pixelated` and no glow. Reduced motion swaps it for the static `studio-logo.png` mark.
* **Emblem Glow Ban:** No emblem instance (hero, header, footer, reduced-motion mark) carries a `drop-shadow`/`filter` glow — owner decision: a drop-shadow follows the image box (the animated loop's transparent frame, or the static mark's plate) instead of hugging the leaf. Header/footer emblems use `pixelated` only; the `/play` loading emblem keeps its approved live-state pulse.
* **Error Reaction Art (`studio-mascot-alarmed.png`, `studio-mascot-sheepish.png`, `studio-mascot-pensive.png`):** 128x128 transparent pixel-art mascot reactions drawn at 64px inside error messages (auth toast, import, reset, revoke). One random reaction per error appearance, never the reaction shown on the previous appearance; decorative (`alt=""`, `aria-hidden`) so the message text alone carries `role="alert"`.
* **Studio Emblem (`studio-logo.png`):** 72x72 pixel-art crystal leaf mark drawn on a 4x grid (18x18 native) over an opaque near-black rounded plate with transparent corners. Scaled crisp using `image-rendering: pixelated` and `image-rendering: crisp-edges`. Used in the header and footer brand slots, the browser tab icon (`icon` + `apple`), the `/play` loading screen, and as the reduced-motion hero mark.
...
**Accepted consequence (2026-09-21, security review RPD-6.1-SEC-02):** the portal header is a client component that receives only the resolved boolean, so the `/admin` literal and the entry's test ids exist in the public client chunk even though the node renders only for admins. Route denial stays non-observable as a *state* — `/admin` returns the same silent 404 with no redirect and no auth prompt for anonymous, forged, stale, and valid-non-admin requests, and never reveals whether the caller is an admin — so this is recorded rather than fixed; making the entry a server-rendered node would contradict the frozen boolean-prop contract.

**Owner decision 2026-09-21 — the plain 404 is the whole anti-enumeration contract; do not re-open.** Measured on the built worker: the denial responses are NOT byte-identical to an unknown-path 404 — `/admin` returns 7216 B under `<html id="__next_error__">` with the route segment names (`(admin)`, `admin`) in the flight payload, where a same-length unknown path returns 8476 B of the normal `_not-found` document (two same-length unknown paths are byte-identical, so the difference is route match, not path length). Owner ruling: non-admins must not learn the panel exists, and a plain 404 achieves that; byte-identity with an unknown path is explicitly NOT required, so neither a differential e2e nor a code change is to be filed for it. This decision is recorded here, in the spec's owner amendments, and in `AGENTS.md` → Known pitfalls precisely because it keeps being re-raised.
