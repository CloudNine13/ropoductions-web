...
* **Animated Studio Emblem (`studio-logo-animated.gif`):** 128x128 transparent 27-frame pixel-art loop of the crystal leaf, hero slot only, rendered at 56px with `.pixelated` and no glow. Reduced motion swaps it for the static mark.
* **Emblem Glow Ban:** No emblem instance (hero, header, footer, reduced-motion mark) carries a `drop-shadow`/`filter` glow — owner decision: the emerald glow renders as a halo around the emblem's transparent box rather than around the leaf. Header/footer emblems use `pixelated` only; the `/play` loading emblem keeps its approved live-state pulse.
...
* **Studio Emblem (`studio-logo.webp`):** 64x64 pixel-art crystal leaf mark. Scaled crisp using `image-rendering: pixelated` and `image-rendering: crisp-edges`. Used in header brand slot, favicon, loading screens, and as the reduced-motion hero mark.
